  ...
  <div class="app-container">
    ...
    <el-dialog :close-on-click-modal="false" :before-close="crud.cancelCU" :visible.sync="crud.status.cu > 0" :title="crud.status.title" append-to-body width="600px">
      <el-form ref="form" :model="form" :rules="rules" size="small" label-width="100px">
        ...
        <el-form-item label="执行方法" prop="invokeTarget">
          <el-input v-model="form.invokeTarget" style="width: 460px;" />
        </el-form-item>
        <el-form-item label="参数内容">
          <el-input v-model="form.jobParams" style="width: 460px;" />
        </el-form-item>
    ...
    <el-table ref="table" v-loading="crud.loading" :data="crud.data" style="width: 100%;" @selection-change="crud.selectionChangeHandler">
      ...
      <el-table-column v-if="columns.visible('invokeTarget')" :show-overflow-tooltip="true" prop="invokeTarget" width="90px" label="执行方法" />
      <el-table-column v-if="columns.visible('jobParams')" :show-overflow-tooltip="true" prop="jobParams" width="80px" label="参数" />
      ...
      <el-table-column v-if="columns.visible('status')" :show-overflow-tooltip="true" prop="status" width="90px" label="状态">
        <template slot-scope="scope">
          <el-tag :type="scope.row.status ? 'warning' : 'success'">{{ scope.row.status ? '已暂停' : '运行中' }}</el-tag>
        </template>
      </el-table-column>
...
<script>
import crudJob from '@/api/tools/timing'
...
// crud交由presenter持有
const defaultCrud = CRUD({ title: '定时任务', url: 'tools/timing', crudMethod: { ...crudJob }})
const defaultForm = { jobName: null, invokeTarget: null, jobParams: null, cronExpression: null, status: 0, remark: null }
export default {
  name: 'Timing',
